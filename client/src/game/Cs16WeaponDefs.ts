import { Weapon } from './Weapon.js';

/**
 * Counter-Strike 1.6 Weapon Definitions - 1:1 Parameters
 */
export const CS16_WEAPON_DEFINITIONS: Record<string, Weapon> = {
  // ============= PISTOLS =============
  glock: new Weapon({
    id: 'glock',
    name: 'Glock 18',
    displayName: 'Glock 18',
    damage: 25,
    fireRate: 6.67,
    magazineSize: 20,
    reloadTime: 2.2,
    spread: 0.08,
    projectileSpeed: 50,
    switchTime: 0.24,
    range: 48,
    reserveAmmo: 120,
    armorPenetration: 0.47,
    adsSpreadMultiplier: 0.65,
  }),

  usp: new Weapon({
    id: 'usp',
    name: 'USP .45',
    displayName: 'USP .45',
    damage: 34,
    fireRate: 5.5,
    magazineSize: 12,
    reloadTime: 2.0,
    spread: 0.06,
    projectileSpeed: 55,
    switchTime: 0.25,
    range: 52,
    reserveAmmo: 100,
    armorPenetration: 0.5,
    adsSpreadMultiplier: 0.55,
  }),

  p228: new Weapon({
    id: 'p228',
    name: 'P228',
    displayName: 'P228',
    damage: 32,
    fireRate: 5.0,
    magazineSize: 13,
    reloadTime: 2.2,
    spread: 0.07,
    projectileSpeed: 52,
    switchTime: 0.25,
    range: 50,
    reserveAmmo: 52,
    armorPenetration: 0.5,
    adsSpreadMultiplier: 0.58,
  }),

  deagle: new Weapon({
    id: 'deagle',
    name: 'Desert Eagle',
    displayName: 'Desert Eagle',
    damage: 54,
    fireRate: 2.5,
    magazineSize: 7,
    reloadTime: 2.2,
    spread: 0.12,
    projectileSpeed: 70,
    switchTime: 0.3,
    range: 65,
    reserveAmmo: 35,
    armorPenetration: 0.93,
    adsSpreadMultiplier: 0.42,
  }),

  fiveseven: new Weapon({
    id: 'fiveseven',
    name: 'Five-SeveN',
    displayName: 'Five-SeveN',
    damage: 20,
    fireRate: 5.0,
    magazineSize: 20,
    reloadTime: 2.7,
    spread: 0.012,
    projectileSpeed: 58,
    switchTime: 0.26,
    range: 55,
    reserveAmmo: 100,
    armorPenetration: 0.91,
    adsSpreadMultiplier: 0.58,
  }),

  dual_berettas: new Weapon({
    id: 'dual_berettas',
    name: 'Dual Elites',
    displayName: 'Dual Elites',
    damage: 36,
    fireRate: 5.0,
    magazineSize: 30,
    reloadTime: 4.5,
    spread: 0.012,
    projectileSpeed: 48,
    switchTime: 0.26,
    range: 52,
    reserveAmmo: 120,
    armorPenetration: 0.45,
    adsSpreadMultiplier: 0.5,
  }),

  // ============= SMGS =============
  mp5: new Weapon({
    id: 'mp5',
    name: 'MP5 Navy',
    displayName: 'MP5 Navy',
    damage: 26,
    fireRate: 10.0,
    magazineSize: 30,
    reloadTime: 2.1,
    spread: 0.1,
    projectileSpeed: 62,
    switchTime: 0.28,
    range: 48,
    reserveAmmo: 120,
    armorPenetration: 0.6,
    adsSpreadMultiplier: 0.65,
  }),

  tmp: new Weapon({
    id: 'tmp',
    name: 'TMP',
    displayName: 'TMP',
    damage: 20,
    fireRate: 11.0,
    magazineSize: 30,
    reloadTime: 2.0,
    spread: 0.09,
    projectileSpeed: 60,
    switchTime: 0.26,
    range: 40,
    reserveAmmo: 120,
    armorPenetration: 0.45,
    adsSpreadMultiplier: 0.62,
  }),

  p90: new Weapon({
    id: 'p90',
    name: 'P90',
    displayName: 'P90',
    damage: 24,
    fireRate: 12.5,
    magazineSize: 50,
    reloadTime: 2.6,
    spread: 0.11,
    projectileSpeed: 66,
    switchTime: 0.3,
    range: 52,
    reserveAmmo: 100,
    armorPenetration: 0.69,
    adsSpreadMultiplier: 0.7,
  }),

  mac10: new Weapon({
    id: 'mac10',
    name: 'MAC-10',
    displayName: 'MAC-10',
    damage: 25,
    fireRate: 12.5,
    magazineSize: 30,
    reloadTime: 2.2,
    spread: 0.13,
    projectileSpeed: 60,
    switchTime: 0.26,
    range: 40,
    reserveAmmo: 100,
    armorPenetration: 0.45,
    adsSpreadMultiplier: 0.7,
  }),

  ump45: new Weapon({
    id: 'ump45',
    name: 'UMP-45',
    displayName: 'UMP-45',
    damage: 30,
    fireRate: 8.5,
    magazineSize: 25,
    reloadTime: 2.3,
    spread: 0.1,
    projectileSpeed: 58,
    switchTime: 0.3,
    range: 46,
    reserveAmmo: 100,
    armorPenetration: 0.65,
    adsSpreadMultiplier: 0.68,
  }),

  // ============= SHOTGUNS =============
  m3: new Weapon({
    id: 'm3',
    name: 'M3 Super 90',
    displayName: 'M3 Super 90',
    damage: 22,
    fireRate: 1.2,
    magazineSize: 8,
    reloadTime: 2.8,
    spread: 0.25,
    projectileSpeed: 40,
    switchTime: 0.45,
    range: 26,
    reserveAmmo: 32,
    armorPenetration: 0.3,
    adsSpreadMultiplier: 0.62,
    pellets: 8,
  }),

  xm1014: new Weapon({
    id: 'xm1014',
    name: 'XM1014',
    displayName: 'XM1014',
    damage: 18,
    fireRate: 2.0,
    magazineSize: 7,
    reloadTime: 3.0,
    spread: 0.28,
    projectileSpeed: 42,
    switchTime: 0.48,
    range: 24,
    reserveAmmo: 32,
    armorPenetration: 0.25,
    adsSpreadMultiplier: 0.65,
    pellets: 8,
  }),

  // ============= RIFLES =============
  ak47: new Weapon({
    id: 'ak47',
    name: 'AK-47',
    displayName: 'AK-47',
    damage: 36,
    fireRate: 7.5,
    magazineSize: 30,
    reloadTime: 2.35,
    spread: 0.075,
    projectileSpeed: 80,
    switchTime: 0.42,
    range: 90,
    reserveAmmo: 90,
    armorPenetration: 0.78,
    adsSpreadMultiplier: 0.55,
  }),

  m4a1: new Weapon({
    id: 'm4a1',
    name: 'M4A1',
    displayName: 'M4A1',
    damage: 33,
    fireRate: 8.0,
    magazineSize: 30,
    reloadTime: 2.35,
    spread: 0.065,
    projectileSpeed: 82,
    switchTime: 0.42,
    range: 95,
    reserveAmmo: 90,
    armorPenetration: 0.7,
    adsSpreadMultiplier: 0.52,
  }),

  sg552: new Weapon({
    id: 'sg552',
    name: 'SG 552',
    displayName: 'SG 552',
    damage: 34,
    fireRate: 8.5,
    magazineSize: 30,
    reloadTime: 2.4,
    spread: 0.07,
    projectileSpeed: 82,
    switchTime: 0.44,
    range: 95,
    reserveAmmo: 90,
    armorPenetration: 0.76,
    adsSpreadMultiplier: 0.48,
  }),

  aug: new Weapon({
    id: 'aug',
    name: 'AUG',
    displayName: 'AUG',
    damage: 32,
    fireRate: 8.0,
    magazineSize: 30,
    reloadTime: 2.35,
    spread: 0.06,
    projectileSpeed: 82,
    switchTime: 0.42,
    range: 95,
    reserveAmmo: 90,
    armorPenetration: 0.73,
    adsSpreadMultiplier: 0.45,
  }),

  galil: new Weapon({
    id: 'galil',
    name: 'Galil',
    displayName: 'Galil',
    damage: 30,
    fireRate: 9.0,
    magazineSize: 35,
    reloadTime: 2.25,
    spread: 0.08,
    projectileSpeed: 76,
    switchTime: 0.38,
    range: 78,
    reserveAmmo: 105,
    armorPenetration: 0.72,
    adsSpreadMultiplier: 0.6,
  }),

  famas: new Weapon({
    id: 'famas',
    name: 'FAMAS',
    displayName: 'FAMAS',
    damage: 30,
    fireRate: 10.0,
    magazineSize: 25,
    reloadTime: 2.3,
    spread: 0.07,
    projectileSpeed: 78,
    switchTime: 0.38,
    range: 80,
    reserveAmmo: 75,
    armorPenetration: 0.7,
    adsSpreadMultiplier: 0.58,
  }),

  // ============= SNIPERS =============
  scout: new Weapon({
    id: 'scout',
    name: 'Scout',
    displayName: 'Scout',
    damage: 65,
    fireRate: 2.0,
    magazineSize: 10,
    reloadTime: 3.0,
    spread: 0.04,
    projectileSpeed: 110,
    switchTime: 0.56,
    range: 120,
    reserveAmmo: 60,
    armorPenetration: 0.96,
    adsSpreadMultiplier: 0.12,
  }),

  awp: new Weapon({
    id: 'awp',
    name: 'AWP',
    displayName: 'AWP',
    damage: 115,
    fireRate: 1.25,
    magazineSize: 10,
    reloadTime: 3.3,
    spread: 0.02,
    projectileSpeed: 120,
    switchTime: 0.64,
    range: 130,
    reserveAmmo: 30,
    armorPenetration: 0.98,
    adsSpreadMultiplier: 0.08,
  }),

  sg550: new Weapon({
    id: 'sg550',
    name: 'SG 550',
    displayName: 'SG 550',
    damage: 70,
    fireRate: 3.5,
    magazineSize: 30,
    reloadTime: 3.2,
    spread: 0.03,
    projectileSpeed: 105,
    switchTime: 0.6,
    range: 115,
    reserveAmmo: 90,
    armorPenetration: 0.95,
    adsSpreadMultiplier: 0.15,
  }),

  g3sg1: new Weapon({
    id: 'g3sg1',
    name: 'G3SG1',
    displayName: 'G3SG1',
    damage: 75,
    fireRate: 3.0,
    magazineSize: 20,
    reloadTime: 3.2,
    spread: 0.035,
    projectileSpeed: 105,
    switchTime: 0.6,
    range: 115,
    reserveAmmo: 60,
    armorPenetration: 0.95,
    adsSpreadMultiplier: 0.18,
  }),

  // ============= MACHINE GUN =============
  m249: new Weapon({
    id: 'm249',
    name: 'M249',
    displayName: 'M249',
    damage: 32,
    fireRate: 10.5,
    magazineSize: 100,
    reloadTime: 5.0,
    spread: 0.12,
    projectileSpeed: 76,
    switchTime: 0.6,
    range: 80,
    reserveAmmo: 200,
    armorPenetration: 0.8,
    adsSpreadMultiplier: 0.75,
  }),

  // ============= MELEE =============
  knife: new Weapon({
    id: 'knife',
    name: 'Knife',
    displayName: 'Knife',
    damage: 55,
    fireRate: 1.8,
    magazineSize: 1,
    reloadTime: 0,
    spread: 0,
    projectileSpeed: 0,
    ammoConsumed: false,
    isMelee: true,
    switchTime: 0.18,
    range: 2.35,
    reserveAmmo: 0,
    armorPenetration: 0.2,
    adsSpreadMultiplier: 1,
  }),

  // ============= EQUIPMENT =============
  hegrenade: new Weapon({
    id: 'hegrenade',
    name: 'HE Grenade',
    displayName: 'HE Grenade',
    damage: 100,
    fireRate: 0.5,
    magazineSize: 1,
    reloadTime: 0,
    spread: 0,
    projectileSpeed: 30,
    switchTime: 0.2,
    range: 50,
    reserveAmmo: 0,
    armorPenetration: 1,
    adsSpreadMultiplier: 1,
  }),
};
