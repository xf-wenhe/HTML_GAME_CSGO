...
export function readInfernoGeneratedMeshResource(modulePath) {
  const source = fs.readFileSync(modulePath, 'utf8');
  return parseInfernoGeneratedMeshModule(source);
}

export function parseInfernoGeneratedMeshModule(source) {
  const match = source.match(/export\s+const\s+INFERNO_WORLD_MESH_RESOURCE[\s\S]*?=\s*([\s\S]*);\s*$/);
  if (!match) {
    throw new Error('Generated Inferno mesh module does not export INFERNO_WORLD_MESH_RESOURCE.');
  }
  ...
  try {
    ...
    throw new Error(`Generated Inferno mesh module does not contain valid JSON resource: ${error instanceof Error ? error.message : String(error)}`);
  }
}

export function verifyInfernoGeneratedMeshResource(resource) {
  if (!resource) {
    throw new Error('Inferno generated mesh resource is null; run inferno:import with a legal CS1.6 de_inferno.bsp or de_inferno.map first.');
  }

  if (resource.schema !== 'fps-web-game/inferno-world-mesh/v1') {
    throw new Error(`Unsupported Inferno generated mesh schema: ${resource.schema}`);
  }
  ...
  if (
    ...
    throw new Error('Inferno generated mesh must be backed by a GoldSrc BSP30 or MAP source.');
  }

  if (path.basename(resource.source.path ?? '').toLowerCase() !== `de_inferno.${sourceKind}`) {
    throw new Error(`Inferno generated mesh source path must be named de_inferno.${sourceKind}.`);
  }

  if (!/^[a-f0-9]{64}$/i.test(resource.source.sha256 ?? '')) {
    throw new Error('Inferno generated mesh source must include a SHA-256 fingerprint.');
  }

  if (resource.source.manifest?.sha256 !== resource.source.sha256) {
    throw new Error('Inferno generated mesh source SHA-256 does not match the source manifest.');
  }

  const geometry = resource.source?.manifest?.geometry;
  if ((geometry?.worldMeshVertexCount ?? 0) <= 0 || (geometry?.worldMeshTriangleCount ?? 0) <= 0) {
    throw new Error('Inferno generated mesh manifest is missing world mesh counts.');
  }

  if ((geometry?.exportedMeshVertexCount ?? 0) <= 0 || (geometry?.exportedMeshTriangleCount ?? 0) <= 0) {
    throw new Error('Inferno generated mesh manifest is missing exported BSP model mesh counts.');
  }
  ...
  if ((geometry?.collisionMeshVertexCount ?? geometry?.exportedMeshVertexCount ?? 0) <= 0 || (geometry?.collisionMeshTriangleCount ?? geometry?.exportedMeshTriangleCount ?? 0) <= 0) {
    throw new Error('Inferno generated mesh manifest is missing collision mesh counts.');
  }

  if (!Array.isArray(geometry?.modelMeshes) || geometry.modelMeshes.length <= 0) {
    throw new Error('Inferno generated mesh manifest is missing BSP model mesh manifests.');
  }

  if (!Array.isArray(geometry?.collision?.worldHullSummaries)) {
    throw new Error('Inferno generated mesh manifest is missing collision hull summaries.');
  }

  if (!Array.isArray(geometry?.collision?.modelHullSummaries)) {
    throw new Error('Inferno generated mesh manifest is missing per-model collision hull summaries.');
  }
  ...
  if ((entities?.entityCount ?? 0) <= 0) {
    throw new Error('Inferno generated mesh manifest is missing source entity data.');
  }
  ...
  if (tSpawnCount <= 0 || ctSpawnCount <= 0) {
    throw new Error('Inferno generated mesh manifest is missing source T/CT spawn entities.');
  }

  if (!Array.isArray(entities.bombTargets) || entities.bombTargets.length < 2) {
    throw new Error('Inferno generated mesh manifest is missing source A/B bomb target entities.');
  }

  if (!Array.isArray(resource.mesh?.positions) || resource.mesh.positions.length !== geometry.exportedMeshVertexCount) {
    throw new Error('Inferno generated mesh positions do not match the exported BSP model mesh vertex count.');
  }

  if (!Array.isArray(resource.mesh?.indices) || resource.mesh.indices.length !== geometry.exportedMeshTriangleCount * 3) {
    throw new Error('Inferno generated mesh indices do not match the exported BSP model mesh triangle count.');
  }
  ...
  if (!Array.isArray(collisionMesh.positions) || collisionMesh.positions.length !== collisionVertexCount) {
    throw new Error('Inferno generated collision mesh positions do not match the source collision mesh vertex count.');
  }

  if (!Array.isArray(collisionMesh.indices) || collisionMesh.indices.length !== collisionTriangleCount * 3) {
    throw new Error('Inferno generated collision mesh indices do not match the source collision mesh triangle count.');
  }
...
function assertExportedModelHullIntegrity(geometry, sourceKind) {
  ...
  if (exportedHullSummaries.length !== exportedModelIndexes.size) {
    throw new Error('Inferno generated mesh manifest is missing collision summaries for exported source models.');
  }
  ...
  for (const modelSummary of exportedHullSummaries) {
    for (const hull of modelSummary.hulls ?? []) {
      if ((hull.missingNodes?.length ?? 0) > 0 || (hull.cycles?.length ?? 0) > 0) {
        throw new Error(`Inferno generated mesh collision hull ${hull.hull} for model ${modelSummary.modelIndex} has unresolved clipnode references.`);
      }
    }

    if (!modelSummary.hulls?.some(hull => (hull.contents?.solid ?? 0) > 0)) {
      throw new Error(`Inferno generated mesh exported model ${modelSummary.modelIndex} has no solid collision hull contents.`);
    }
  }

  if (sourceKind === 'map' && (geometry.collision?.brushSolidCount ?? 0) <= 0) {
    throw new Error('Inferno generated MAP mesh manifest is missing solid brush collision evidence.');
  }
}
